...
import type { Country, InitialState } from './types';
...
export function fetchData() {
  return async (dispatch: Dispatch, getState: () => InitialState) => {
    ...
    try {
      const response = await fetch('https://restcountries.com/v2/all?fields=name,population,flags,currencies,region,nativeName,borders,capital,timezones,languages');
